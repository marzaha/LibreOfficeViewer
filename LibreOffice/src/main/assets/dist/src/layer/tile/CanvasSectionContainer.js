...
var CanvasSectionObject = /** @class */ (function () {
    function CanvasSectionObject(options) {
        ...
        this.name = null;
        this.backgroundColor = null; // Defult is null (container's background color will be used).
        ...
        this.documentObject = false; // If true, the section is a document object.
        ...
        this.origSizeHint = undefined; // This is used to preserve the original size provided on construct.
        this.expand = new Array(0);
        this.isLocated = false; // Location and size of the section computed yet?
...
var CanvasSectionContainer = /** @class */ (function () {
    function CanvasSectionContainer(canvasDOMElement, disableDrawing) {
        /*
        ...
        */
        ...
        this.clearColor = '#f8f9fa'; // '#f8f9fa';
        this.touchEventInProgress = false; // This prevents multiple calling of mouse down and up events.
        this.testing = false; // If this set to true, container will create a div element for every section. So, cypress tests can find where to click etc.
        this.lowestPropagatedBoundSection = null; // Event propagating to bound sections. The first section which stops propagating and the sections those are on top of that section, get the event.
        this.targetSection = null;
        this.activeSection = null;
        this.scrollLineHeight = 30; // This will be overridden.
        this.mouseIsInside = false;
        this.inZoomAnimation = false;
        this.zoomChanged = false;
        this.documentAnchorSectionName = null; // This section's top left point declares the point where document starts.
        this.documentAnchor = null; // This is the point where document starts inside canvas element. Initial value shouldn't be [0, 0].
        // Above 2 properties can be used with documentBounds.
        this.drawingPaused = 0;
        this.drawingEnabled = true;
        this.dirty = false;
        this.sectionsDirty = false;
        this.paintedEver = false;
        // For window sections.
        this.windowSectionList = [];
        // Below variables are related to animation feature.
        this.animatingSectionName = null; // The section that called startAnimating function. This variable is null when animations are not running.
        this.lastFrameStamp = null;
        this.continueAnimating = null;
        this.frameCount = null; // Frame count of the current animation.
        this.duration = null; // Duration for the animation.
        this.elapsedTime = null; // Time that passed since the animation started.
    ...
    CanvasSectionContainer.prototype.clearCanvas = function () {
        this.context.fillStyle = this.clearColor;
        this.context.clearRect(0, 0, this.canvas.width, this.canvas.height);
        this.context.fillRect(0, 0, this.canvas.width, this.canvas.height);
    };
    CanvasSectionContainer.prototype.getContext = function () {
        return this.context;
    };
    CanvasSectionContainer.prototype.setDocumentAnchorSection = function (sectionName) {
        var section = this.getSectionWithName(sectionName);
        if (section) {
    ...
    CanvasSectionContainer.prototype.getDocumentAnchorSection = function () {
        return this.getSectionWithName(this.documentAnchorSectionName);
    };
    CanvasSectionContainer.prototype.getViewSize = function () {
    ...
    CanvasSectionContainer.prototype.setInZoomAnimation = function (inZoomAnimation) {
        this.inZoomAnimation = inZoomAnimation;
    };
    CanvasSectionContainer.prototype.isInZoomAnimation = function () {
        return this.inZoomAnimation;
    };
    CanvasSectionContainer.prototype.setZoomChanged = function (zoomChanged) {
        this.zoomChanged = zoomChanged;
    };
    CanvasSectionContainer.prototype.isZoomChanged = function () {
        return this.zoomChanged;
    };
    CanvasSectionContainer.prototype.drawingAllowed = function () {
    ...
    CanvasSectionContainer.prototype.disableDrawing = function () {
        this.drawingEnabled = false;
    };
    CanvasSectionContainer.prototype.enableDrawing = function () {
    ...
    CanvasSectionContainer.prototype.pauseDrawing = function () {
        if (this.drawingPaused++ === 0) {
            this.dirty = false;
        }
    };
    ...
    CanvasSectionContainer.prototype.resumeDrawing = function (topLevel) {
        var wasNonZero = this.drawingPaused !== 0;
        if (topLevel)
            this.drawingPaused = 0;
    ...
    CanvasSectionContainer.prototype.paintOnResumeOrEnable = function () {
        if (this.sectionsDirty) {
            this.updateBoundSectionLists();
            this.reNewAllSections(false);
            this.sectionsDirty = false;
        }
        var scrollSection = this.getSectionWithName(L.CSections.Scroll.name);
        if (scrollSection)
    ...
    CanvasSectionContainer.prototype.setDirty = function () {
        this.dirty = true;
    };
    /**
     * IE11 doesn't support Array.includes, use replacement
    */
    CanvasSectionContainer.prototype.arrayIncludes = function (array, element) {
    ...
    CanvasSectionContainer.prototype.getDocumentBottomRight = function () {
        return [this.documentBottomRight[0], this.documentBottomRight[1]];
    };
    // Returns top-left and bottom-right coordinates respectively.
    CanvasSectionContainer.prototype.getDocumentBounds = function () {
        return [this.documentTopLeft[0], this.documentTopLeft[1], this.documentBottomRight[0], this.documentBottomRight[1]];
    };
    CanvasSectionContainer.prototype.getDocumentSize = function () {
        return [this.documentBottomRight[0] - this.documentTopLeft[0], this.documentBottomRight[1] - this.documentTopLeft[1]];
    };
    CanvasSectionContainer.prototype.isDocumentObjectVisible = function (section) {
        if ((section.position[0] >= this.documentTopLeft[0] && section.position[0] <= this.documentBottomRight[0] ||
    ...
    CanvasSectionContainer.prototype.targetBoundSectionListContains = function (sectionName) {
        if (!this.targetSection)
            return false;
        else {
            var section = this.getSectionWithName(this.targetSection);
            if (section && section.boundsList) {
                for (var i = 0; i < section.boundsList.length; i++) {
    ...
    CanvasSectionContainer.prototype.setDocumentBounds = function (points) {
    ...
    CanvasSectionContainer.prototype.updateBoundSectionList = function (section, sectionList) {
        if (sectionList === void 0) { sectionList = null; }
        if (sectionList === null)
            sectionList = new Array(0);
        sectionList.push(section);
        var tempSectionList = new Array(0);
    ...
    CanvasSectionContainer.prototype.orderBoundsList = function (section) {
        // According to zIndex & drawingOrder.
        for (var i = 0; i < section.boundsList.length - 1; i++) {
        ...
        for (var i = section.boundsList.length - 1; i > -1; i--) {
    ...
    CanvasSectionContainer.prototype.updateBoundSectionLists = function () {
        for (var i = 0; i < this.sections.length; i++) {
    ...
    CanvasSectionContainer.prototype.propagateOnClick = function (section, position, e) {
        this.targetSection = section.name;
        var propagate = true;
        var windowPosition = position ? [position[0] + section.myTopLeft[0], position[1] + section.myTopLeft[1]] : null;
        for (var j = 0; j < this.windowSectionList.length; j++) {
    ...
    CanvasSectionContainer.prototype.propagateOnDoubleClick = function (section, position, e) {
        this.targetSection = section.name;
        var propagate = true;
        var windowPosition = position ? [position[0] + section.myTopLeft[0], position[1] + section.myTopLeft[1]] : null;
        for (var j = 0; j < this.windowSectionList.length; j++) {
    ...
    CanvasSectionContainer.prototype.propagateOnMouseLeave = function (section, position, e) {
        this.targetSection = section.name;
        var windowPosition = position ? [position[0] + section.myTopLeft[0], position[1] + section.myTopLeft[1]] : null;
    ...
    CanvasSectionContainer.prototype.propagateOnMouseEnter = function (section, position, e) {
        this.targetSection = section.name;
    ...
    CanvasSectionContainer.prototype.propagateOnMouseMove = function (section, position, dragDistance, e) {
        this.targetSection = section.name;
        var propagate = true;
        var windowPosition = position ? [position[0] + section.myTopLeft[0], position[1] + section.myTopLeft[1]] : null;
        for (var j = 0; j < this.windowSectionList.length; j++) {
    ...
    CanvasSectionContainer.prototype.propagateOnLongPress = function (section, position, e) {
        this.targetSection = section.name;
        var propagate = true;
        var windowPosition = position ? [position[0] + section.myTopLeft[0], position[1] + section.myTopLeft[1]] : null;
        for (var j = 0; j < this.windowSectionList.length; j++) {
    ...
    CanvasSectionContainer.prototype.propagateOnMouseDown = function (section, position, e) {
        this.targetSection = section.name;
        var propagate = true;
        var windowPosition = position ? [position[0] + section.myTopLeft[0], position[1] + section.myTopLeft[1]] : null;
        for (var j = 0; j < this.windowSectionList.length; j++) {
    ...
    CanvasSectionContainer.prototype.propagateOnMouseUp = function (section, position, e) {
        this.targetSection = section.name;
        var propagate = true;
        var windowPosition = position ? [position[0] + section.myTopLeft[0], position[1] + section.myTopLeft[1]] : null;
        for (var j = 0; j < this.windowSectionList.length; j++) {
    ...
    CanvasSectionContainer.prototype.propagateOnContextMenu = function (section) {
        this.targetSection = section.name;
        var propagate = true;
        for (var j = 0; j < this.windowSectionList.length; j++) {
            var windowSection = this.windowSectionList[j];
    ...
    CanvasSectionContainer.prototype.propagateOnMouseWheel = function (section, position, delta, e) {
        this.targetSection = section.name;
        var propagate = true;
        var windowPosition = position ? [position[0] + section.myTopLeft[0], position[1] + section.myTopLeft[1]] : null;
        for (var j = 0; j < this.windowSectionList.length; j++) {
    ...
    CanvasSectionContainer.prototype.propagateOnMultiTouchStart = function (section, e) {
        this.targetSection = section.name;
        var propagate = true;
        for (var j = 0; j < this.windowSectionList.length; j++) {
            var windowSection = this.windowSectionList[j];
    ...
    CanvasSectionContainer.prototype.propagateOnMultiTouchMove = function (section, position, distance, e) {
        this.targetSection = section.name;
        var propagate = true;
        var windowPosition = position ? [position[0] + section.myTopLeft[0], position[1] + section.myTopLeft[1]] : null;
        for (var j = 0; j < this.windowSectionList.length; j++) {
    ...
    CanvasSectionContainer.prototype.propagateOnMultiTouchEnd = function (section, e) {
        this.targetSection = section.name;
        var propagate = true;
        for (var j = 0; j < this.windowSectionList.length; j++) {
            var windowSection = this.windowSectionList[j];
    ...
    CanvasSectionContainer.prototype.onContextMenu = function (e) {
        var mousePosition = this.convertPositionToCanvasLocale(e);
        var section = this.findSectionContainingPoint(mousePosition);
        if (section) {
    ...
    CanvasSectionContainer.prototype.onMouseLeave = function (e) {
        ...
        if (!this.draggingSomething) {
            if (this.sectionUnderMouse !== null) {
                var section = this.getSectionWithName(this.sectionUnderMouse);
                if (section)
    ...
    CanvasSectionContainer.prototype.onMouseEnter = function (e) {
        this.mouseIsInside = true;
        for (var i = 0; i < this.windowSectionList.length; i++) {
    ...
    CanvasSectionContainer.prototype.onResize = function (newWidth, newHeight) {
        ...
        var cssWidth = newWidth / app.dpiScale; // NB. beware
        var cssHeight = newHeight / app.dpiScale;
    ...
    CanvasSectionContainer.prototype.createUpdateSingleDivElement = function (section) {
        var bcr = this.canvas.getBoundingClientRect();
        var element = document.getElementById('test-div-' + section.name);
        if (!element) {
    ...
    CanvasSectionContainer.prototype.createUpdateDivElements = function () {
        for (var i = 0; i < this.sections.length; i++) {
    ...
    CanvasSectionContainer.prototype.roundPositionAndSize = function (section) {
    ...
    CanvasSectionContainer.prototype.calculateSectionInitialPosition = function (section, index) {
        if (typeof section.anchor[index] === 'string' || section.anchor[index].length === 1) {
            var anchor = typeof section.anchor[index] === 'string' ? section.anchor[index] : section.anchor[index][0];
            if (index === 0)
            ...
            if (section.anchor[index].length % 2 === 0) { // eslint-disable-line no-lonely-if
                ...
                var count = section.anchor[index].length;
                ...
                if (targetSection) {
                    ...
                    var anchor = section.anchor[index][count - 1];
                    if (index === 0) {
                        return anchor === 'top' ? section.position[1] : (this.bottom - (section.position[1] + section.size[1]));
                    }
                    else {
    ...
    CanvasSectionContainer.prototype.expandSection = function (section) {
    ...
    CanvasSectionContainer.prototype.locateSections = function () {
        for (var i = 0; i < this.sections.length; i++) {
            ...
            if (typeof section.origSizeHint === 'undefined') {
                section.origSizeHint = __spreadArray([], __read(section.size));
            }
        }
        this.documentAnchor = null;
        this.windowSectionList = [];
    ...
    CanvasSectionContainer.prototype.applyDrawingOrders = function () {
        ...
        for (var i = 0; i < this.sections.length - 1; i++) {
            ...
            for (var j = i + 1; j < this.sections.length && this.sections[j].zIndex === zIndex; j++) {
    ...
    CanvasSectionContainer.prototype.setPenPosition = function (section) {
        this.context.setTransform(1, 0, 0, 1, 0, 0);
        this.context.translate(section.myTopLeft[0], section.myTopLeft[1]);
    };
    CanvasSectionContainer.prototype.drawSections = function (frameCount, elapsedTime) {
    ...
    CanvasSectionContainer.prototype.checkNewSectionName = function (options) {
    ...
    CanvasSectionContainer.prototype.checkSectionProperties = function (options) {
    ...
    CanvasSectionContainer.prototype.createSection = function (options, parentSectionName) {
    ...
    CanvasSectionContainer.prototype.addSection = function (newSection) {
    ...
    CanvasSectionContainer.prototype.addSectionFunctions = function (section) {
        section.setDrawingOrder = function (drawingOrder) {
            section.drawingOrder = drawingOrder;
            section.containerObject.updateBoundSectionLists();
            section.containerObject.reNewAllSections();
        };
        section.setZIndex = function (zIndex) {
            section.zIndex = zIndex;
            section.containerObject.updateBoundSectionLists();
            section.containerObject.reNewAllSections();
        };
        section.bindToSection = function (sectionName) {
        ...
        section.startAnimating = function (options) {
        ...
        section.getTestDiv = function () {
            var element = document.getElementById('test-div-' + this.name);
            if (element) {
                return element;
            }
            else {
        ...
        section.isCalcRTL = function () {
            var docLayer = section.sectionProperties.docLayer;
        ...
        if (section.documentObject === true) {
            section.setPosition = function (x, y) {
                x = Math.round(x);
                y = Math.round(y);
                var sectionXcoord = x - section.containerObject.documentTopLeft[0];
                if (section.isCalcRTL()) {
                    ...
                    var docLayer = section.sectionProperties.docLayer;
                    var docSize = docLayer._map.getPixelBoundsCore().getSize();
    ...
    CanvasSectionContainer.prototype.pushSection = function (newSection) {
        // Every section can draw from Point(0, 0), their drawings will be translated to myTopLeft position.
    ...
    CanvasSectionContainer.prototype.removeSection = function (name) {
        ...
        for (var i = 0; i < this.sections.length; i++) {
            if (this.sections[i].name === name) {
                var element = document.getElementById('test-div-' + this.sections[i].name);
    ...
    CanvasSectionContainer.prototype.setAnimatingSectionName = function (sectionName) {
        this.animatingSectionName = sectionName;
    };
    CanvasSectionContainer.prototype.getAnimatingSectionName = function () {
        return this.animatingSectionName;
    };
    CanvasSectionContainer.prototype.animate = function (timeStamp) {
        ...
        if (this.continueAnimating) {
            ...
            for (var i = 0; i < this.stoppingFunctionList.length; i++) {
                this.canvas.removeEventListener(this.stoppingEventTypes[i], this.stoppingFunctionList[i], true);
            }
            var section = this.getSectionWithName(this.getAnimatingSectionName());
            if (section) {
    ...
    CanvasSectionContainer.prototype.createStoppingFunction = function () {
        return function () {
            this.continueAnimating = false;
        }.bind(this);
    };
    // Resets animation duration. Not to be called directly. Instead, use (inside section class) this.resetAnimation()
    CanvasSectionContainer.prototype.resetAnimation = function (sectionName) {
    ...
    CanvasSectionContainer.prototype.stopAnimating = function () {
    ...
    CanvasSectionContainer.prototype.startAnimating = function (sectionName, options) {
        /*
        ...
        */
        if (!this.getAnimatingSectionName()) {
            ...
            this.elapsedTime = 0;
            this.frameCount = 0;
            this.stoppingFunctionList = new Array(0);
            this.stoppingEventTypes = new Array(0);
            if (options.stoppingEvents) {
                for (var i = 0; i < options.stoppingEvents.length; i++) {
